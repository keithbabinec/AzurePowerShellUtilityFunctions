function Send-AppInsightsEventTelemetry
{
    ...
    .PARAMETER EventCustomProperties
        Optionally specify additional custom properties, in the form of a hashtable (key-value pairs) that should be logged in this event.
	...
	.EXAMPLE
        C:\> Send-AppInsightsEventTelemetry -InstrumentationKey <guid> -EventName 'MyEvent1' -EventCustomProperties @{ 'CustomProperty1'='abc'; 'CustomProperty2'='xyz' }
    ...
    Process
    {
		...
		if ($PSBoundParameters.ContainsKey('EventCustomProperties') -and $EventCustomProperties.Count -gt 0)
		{
			$customPropertiesObj = [PSCustomObject]$EventCustomProperties
		}
		else
		{
